...
export class SpyglassSheet {
  ...
  constructor (workbook: WorkBook) {
    if (typeof workbook === 'undefined') { throw new Error('Cannot instantiate Sheet class directly, use the init method instead') }
    if (workbook.SheetNames.length !== 1) { throw new Error('Wrong amount of sheets in workbook') }
  ...
  private getSheetLength (): number {
    let sheetLength = 1
    while (this.readCell(`A${sheetLength}`) !== undefined) { sheetLength++ }
    return --sheetLength
  }
